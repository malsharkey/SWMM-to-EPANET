#!/usr/bin/env python3
"""
swmm_to_epanet_v6.py

Convert a basic EPA SWMM .inp network into an EPANET .inp skeleton.

This version follows EPA SWMM tokenisation first: whitespace separates tokens,
semicolon starts comments, and double-quoted text is kept as one token. It then
adds a controlled salvage layer for real-world GUI/export files where object IDs
with spaces were written without quotes or overflowed SWMM's pretty columns.

The converter normalises EPANET IDs and can write an ID mapping CSV.
...
python swmm_to_epanet_v6.py input_swmm.inp output_epanet.inp --warnings warnings.txt --id-map id_map.csv
"""
...
class IdMapper:
    """Map original SWMM IDs to EPANET-safe IDs.

    EPANET INP files are much less forgiving when object IDs contain spaces or
    punctuation. Rather than losing the original names, this mapper creates safe
    IDs and optionally writes a CSV map for audit/review.
    """
    ...
    def rows(self) -> List[Tuple[str, str]]:
        return sorted(self.original_to_safe.items(), key=lambda kv: kv[1].lower())

    def write_csv(self, path: Path) -> None:
        with path.open("w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow(["original_swmm_id", "epanet_id"])
            for original, safe in self.rows():
                writer.writerow([original, safe])
...
def remove_comment(line: str) -> str:
    """Strip a SWMM comment, respecting double-quoted text.

    EPA SWMM tokenisation treats semicolon as the start of a comment, but quoted
    text is a single token. This keeps semicolons inside quotes instead of
    chopping the row in half like a tiny parser goblin.
    """
    in_quote = False
    out: List[str] = []
    i = 0
...
def tokens(line: str) -> List[str]:
    """Return EPA-SWMM-style tokens.

    SWMM's engine separates tokens with whitespace and keeps double-quoted text
    together. `shlex` is close enough for this purpose after comments are
    stripped. If the row has a broken quote, fall back to whitespace splitting
    and warn later in the section parser rather than crashing.
    """
...
def parse_named_numeric_row(line: str, min_numbers: int, section: str, warnings: List[str]) -> Optional[Tuple[str, List[float]]]:
    """Parse rows of the form Name numeric numeric...

    Primary behaviour matches valid SWMM tokenisation: token 0 is the object ID
    and token 1 is the first numeric field. Salvage behaviour then handles
    GUI/export rows where a multi-word name was written without quotes.
    """
    p = tokens(line)
    if len(p) >= 1 + min_numbers and to_float(p[1]) is not None:
        nums: List[float] = []
        for token in p[1:]:
            v = to_float(token)
            if v is None:
    ...
    parsed = parse_numeric_suffix_row(line, min_numbers=min_numbers)
    if parsed:
        return parsed
...
def parse_junctions(sec: Optional[RawSection], warnings: List[str]) -> Dict[str, float]:
    ...
    if not sec:
        return out
    for line in sec.lines:
        parsed = parse_named_numeric_row(line, min_numbers=1, section="JUNCTIONS", warnings=warnings)
        if not parsed:
            continue
        name, nums = parsed
        out[name] = nums[0]
...
def parse_outfalls(sec: Optional[RawSection], warnings: List[str]) -> Dict[str, float]:
    ...
    for line in sec.lines:
        ...
        # Strict SWMM form: Name Elevation Type ...
        if len(p) >= 3 and to_float(p[1]) is not None and p[2].upper() in OUTFALL_TYPES:
            name = p[0]
            elev = float(p[1])
            out_type = p[2].upper()
            head = elev
            if out_type == "FIXED" and len(p) >= 4:
                fixed = to_float(p[3])
        ...
        # Salvage form: multi word name Elevation Type ...
        # Find an outfall type whose immediately preceding token is numeric.
        candidates = []
...
def parse_storage(sec: Optional[RawSection], warnings: List[str]) -> Dict[str, Tuple[float, float, float, float]]:
    ...
    if not sec:
        return out
    for line in sec.lines:
        parsed = parse_named_numeric_row(line, min_numbers=3, section="STORAGE", warnings=warnings)
        if not parsed:
            continue
        name, nums = parsed
...
def convert(model: SwmmModel, *, include_pumps: bool = True, include_non_circular: bool = False) -> Tuple[str, List[str], IdMapper]:
    warnings: List[str] = []
    ids = IdMapper()
...
def convert_file(
    ...
    *,
    include_non_circular: bool = False,
    include_pumps: bool = True,
    id_map_path: Optional[Path] = None,
) -> List[str]:
    model = read_swmm(input_path)
    text, warnings, ids = convert(model, include_pumps=include_pumps, include_non_circular=include_non_circular)
    output_path.write_text(text, encoding="utf-8")
    if warning_path:
        warning_path.write_text("\n".join(warnings) + "\n", encoding="utf-8")
    if id_map_path:
        ids.write_csv(id_map_path)
    return warnings
...
def main() -> None:
    ...
    parser.add_argument("--id-map", type=Path, help="Optional CSV mapping original SWMM IDs to EPANET-safe IDs")
    ...
    warnings = convert_file(
        ...
        output,
        args.warnings,
        include_non_circular=args.include_non_circular,
        include_pumps=not args.no_pumps,
    ...
    if args.id_map:
        print(f"Wrote ID map:      {args.id_map}")
    print("\nSummary")
    print("-------")
